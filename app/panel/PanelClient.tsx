...
import { projectDefaults, saveProjectContent, subscribeProjectContent, verifyPanelAccess, type ProjectContent } from "@/app/lib/projectContent";
...
const PANEL_SESSION_KEY = "elys-prime-panel-session";

export default function PanelClient() {
  const [content, setContent] = useState<ProjectContent>(projectDefaults);
  const [saved, setSaved] = useState(false);
  const [authorized, setAuthorized] = useState(() => window.sessionStorage.getItem(PANEL_SESSION_KEY) === "active");
  ...
  useEffect(() => {
    ...
    setIsLoading(true);
    return subscribeProjectContent(
      (nextContent) => {
  ...
  const login = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (!await verifyPanelAccess(accessCode.trim())) {
        setAccessError("Giriş kodu hatalı.");
        return;
      }
      window.sessionStorage.setItem(PANEL_SESSION_KEY, "active");
      setAuthorized(true);
      setAccessCode("");
    } catch {
      setAccessError("Giriş kontrol edilemedi. Firestore bağlantısını ve rules yayınını kontrol edin.");
    } finally {
  ...
  const save = async () => {
    ...
    try {
      ...
      setDataError("Değişiklikler kaydedilemedi. İnternet bağlantısını ve Firestore Rules ayarını kontrol edin.");
    } finally {
      setIsSaving(false);
    }
  };
  ...
  if (!authorized) return <main className="grid min-h-dvh place-items-center bg-[#181a18] px-5 text-[#f6f1eb]">
    <section className="w-full max-w-md border border-white/10 bg-[#202320] p-7 sm:p-10">
      ...
      <p className="mt-3 text-sm leading-6 text-white/45">Devam etmek için ortak panel giriş kodunu yazın.</p>
      <form onSubmit={login} className="mt-8">
        <label className={labelClass}>Giriş kodu<input autoFocus autoComplete="current-password" type="password" className={inputClass} value={accessCode} onChange={(event) => setAccessCode(event.target.value)} /></label>
        {accessError && <p role="alert" className="mt-3 text-xs leading-5 text-red-300">{accessError}</p>}
        <button disabled={isCheckingAccess || !accessCode.trim()} className="mt-5 w-full bg-[#d8b792] px-5 py-4 text-[10px] font-bold uppercase tracking-[.13em] text-[#181a18] transition hover:bg-white disabled:cursor-not-allowed disabled:opacity-45">{isCheckingAccess ? "Kontrol ediliyor..." : "Panele gir"}</button>
      </form>
      <a href="/" className="mt-5 inline-flex items-center gap-2 text-[10px] font-bold uppercase tracking-[.12em] text-white/45 hover:text-white"><ChevronLeft size={14}/> Siteye dön</a>
    </section>
  </main>;
