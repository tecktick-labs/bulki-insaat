"use client";
import { ArrowUp, ArrowUpRight, Building2, FileText, Mail, MapPin, MessageCircle, Newspaper, Phone } from "lucide-react";
import Link from "next/link";
import type { ContactPerson } from "@/lib/project-content";

function digits(phone: string) { return phone.replace(/[^\d]/g, ""); }

export default function ContactSection({ projectName, companyName, email, contacts, latitude, longitude }: { projectName: string; companyName: string; email: string; contacts: ContactPerson[]; latitude: number; longitude: number }) {
  ...
  return <section id="iletisim" className="h-dvh snap-start snap-always overflow-hidden bg-[#171917] text-white">
    <div className="mx-auto flex h-full max-w-[1440px] flex-col px-5 pb-5 pt-24 sm:px-9 sm:pb-8 lg:px-14 lg:pb-10 lg:pt-28">
      ...
      <footer className="mt-4 grid shrink-0 grid-cols-2 gap-3 border-t border-white/15 pt-4 text-white/50 lg:grid-cols-[1.1fr_auto_auto_auto_auto_auto_auto] lg:items-center">
        ...
        <Link href="/tanitimlar" className="inline-flex items-center gap-2 text-[8px] font-bold uppercase tracking-[.12em] transition-colors hover:text-white"><FileText size={13}/> Tanıtımlar</Link>
        <Link href="/blog" className="inline-flex items-center gap-2 text-[8px] font-bold uppercase tracking-[.12em] transition-colors hover:text-white"><Newspaper size={13}/> Blog</Link>
        ...
        <Link href="/panel" className="inline-flex items-center gap-2 text-[8px] font-bold uppercase tracking-[.12em] transition-colors hover:text-white"><Building2 size={13}/> Yönetim paneli</Link>
